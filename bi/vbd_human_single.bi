model vbd_human {
  ...
  sub parameter {
    ...
    p_vol_transmission[setting] ~ uniform(lower = 0, upper = 50)
  }

  sub initial {
    S[setting,disease] <- (setting == 0 && disease == 0 ? p_initial_susceptible : 1) * p_N_h[setting]
  ...
  sub transition {
    ...
    R0[setting,disease] ~ truncated_gaussian(mean = p_R0[setting,disease], std = p_vol_transmission[setting], lower = 0)

    ode {
      dS[setting,disease]/dt =
      - R0[setting,disease] / p_d_inf[disease] * I[setting,disease] * S[setting,disease] / p_N_h[setting] 

      dE[setting,disease,delta_h_erlang]/dt =
      + (delta_h_erlang == 0 ? R0[setting,disease] / p_d_inf[disease] * I[setting,disease] * S[setting,disease] / p_N_h[setting] : e_delta_h * (1 / p_d_inc[disease]) * E[setting,disease,delta_h_erlang - 1])
  ...
  sub observation {
    ...
    Sero[obs_id] ~ gaussian(mean = R[obs_id % 2,obs_id / 2] / p_N_h[obs_id % 2], std = 0.09 / 3.98)
  }
