...
This document contains the R code necessary to reproduce the results in "Comparative analysis of dengue and Zika outbreaks reveals differences by setting and virus." All code and data used here are in a [github repository](http://github.com/sbfnk/vbd).

## Required packages
```{r packages}
cran_packages <- c("cowplot", "dplyr", "tidyr", "stringi", "msm")
...
```{r r0vgi, cache = FALSE,fig.height = 4}
temp_plot <-
    plot_libbi(read = libbi_results[["vbd_fnh_all"]][["trace"]][["posterior"]],
               prior = libbi_results[["vbd_fnh_all"]][["trace"]][["prior"]],
               model = libbi_results[["vbd_fnh_all"]][["model"]],
...
p_r0vgi <- ggplot(r0gi, aes(x = `italic(G)`)) +
        ...
        facet_wrap(~ obs_id, scales = "free") +
        geom_rect(data = cross_sections, aes(xmin = `italic(G)` - 0.1, xmax = `italic(G)` + 0.1), ymin = -Inf, ymax = Inf, alpha = 0.2, lwd = 3)
p_r0vgi
```
...
In this model, the outbreak starts in a patch which contains a proportion of the population $\phi$ of the population. This and another patch share the same parameters, and exert a force of infection on each other that is reduced by a factor $\sigma$ with respect to the force of infection within each patch.
...
We include this model to show that, in principle, the two-patch model can reproduce the two peaks.
...
```{r r0vgi_patch_zika_yap, cache = FALSE,fig.height = 4}
temp_plot <-
    plot_libbi(read = libbi_results[["vbd_patch_fnh_yap_zika_all"]][["trace"]][["posterior"]],
               prior = libbi_results[["vbd_patch_fnh_yap_zika_all"]][["trace"]][["prior"]],
               model = libbi_results[["vbd_patch_fnh_yap_zika_all"]][["model"]],
