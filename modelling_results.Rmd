...
See instructions in the example above
...
## Two patches (all three outbreak)
...
## Two patches (Zika in Yap only)

We include this model to show that, in principle, the two-patch model can reproduce the two peaks (but )
See instructions in the example above
...
```{r plot_fits_two_patch_zika_yap}
  temp_plot <-
    plot_libbi(read = libbi_results[["vbd_patch_fnh_yap_zika_all"]][["trace"]][["posterior"]]["Cases"],
               ...
               data = data %>% filter(value > 0 & obs_id = "Zika in Yap"),
               density_args = list(adjust = 2),
               ## densities = "histogram",
               extra.aes = list(group = "obs_id"),
               data.colour = "black",
